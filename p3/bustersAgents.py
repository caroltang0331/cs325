...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for eachGhost in livingGhostPositionDistributions:
            #position with the max in belief distribution
            gLoc = eachGhost.argMax()
            #qp.push((ghost location, distance between), the distance between)
            pq.push((gLoc, self.distancer.getDistance(pacmanPosition, gLoc)), self.distancer.getDistance(pacmanPosition, gLoc))
            gloc, gdistance = pq.pop() #closest ghost
            for turn in legal:
                turnDis = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,turn), gloc)
                if (turnDis < gdistance):
                    #best action is this turn, unless another action can reduce the distance more
                    gdistance = turnDis
                    action = turn
            return action
