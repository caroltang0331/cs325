...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        # distribution for new p given old p
        for p in self.legalPositions:
            newDistri = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for pos, prob in newDistri.items():
                # update new p belief
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                #similar like exact inference
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                weight[p] += emissionModel[trueDistance]

            weight.normalize()
            #edge case weights are 0
